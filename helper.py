...
def most_busy_users(df):
    ...
    # Calculate percentage of activity for each user
    new_df = round((df_filtered['user'].value_counts() / df_filtered.shape[0]) * 100, 2).reset_index().rename(
...
def most_common_words(selected_user, df):
    ...
    with open('stop_hinglish.txt', 'r') as f:
        stopwords = set(f.read().split())  # Convert into a set for faster lookup
    ...
    for message in df['message']:
        for word in message.lower().split():
            ...
            if word and word not in stopwords:  # Exclude empty words & stopwords
                words.append(word)
